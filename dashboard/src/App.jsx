...
const s = {
  app: { fontFamily: "'Inter', -apple-system, sans-serif", background: C.bg, minHeight: "100vh", color: C.text },
  nav: {
  ...
  navLogo: { fontSize: "15px", fontWeight: 600, color: C.text },
  navBadge: {
    fontSize: "11px", padding: "2px 8px", borderRadius: "12px",
    background: `${C.green}22`, color: C.green, border: `1px solid ${C.green}44`,
  },
  navLink: { fontSize: "12px", color: C.muted, textDecoration: "none" },
  ...
  grid4: { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: "12px", marginBottom: "28px" },
  ...
  btnDanger: {
    ...
    border: `1px solid ${C.border}`, borderRadius: "6px", fontSize: "11px", cursor: "pointer",
  },
  tag: (color) => ({
  ...
  dlRow: {
    ...
    display: "grid", gridTemplateColumns: "1fr 1fr 80px", gap: "8px", fontSize: "12px",
  },
  pulse: { width: "8px", height: "8px", borderRadius: "50%", background: C.green, display: "inline-block", marginRight: "6px", animation: "pulse 2s infinite" },
};

const statusColor = (s) => s === "completed" ? C.green : s === "failed" || s === "dead" ? C.red : s === "processing" ? C.amber : C.muted;
...
export default function App() {
  ...
  const replayDead = async () => {
    await axios.post(`${API}/dead-letter/replay`);
    fetchAll();
    showToast("Dead tasks requeued!", "success");
  };
  ...
  return (
    <div style={s.app}>
      ...
      <nav style={s.nav}>
        ...
        <a href={`${API}/docs`} target="_blank" rel="noreferrer" style={s.navLink}>API Docs →</a>
      </nav>
      ...
      <main style={s.main}>
        ...
        <div style={{ ...s.card, marginBottom: "28px" }}>
          <p style={s.sectionTitle}>Live Throughput — tasks processed per 10s interval</p>
          {history.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={200}>
              <LineChart data={history} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
                ...
                <Line type="monotone" dataKey="throughput" name="Tasks/interval" stroke={C.blue} strokeWidth={2} dot={false} />
        ...
        <div style={s.grid2}>
          ...
          <div style={s.card}>
            ...
            <input style={s.input} value={trackId} onChange={e => setTrackId(e.target.value)} placeholder="Paste task ID…" />
            ...
            {trackedTask && (
              <div style={{ marginTop: "12px", padding: "12px", background: C.bg, borderRadius: "6px", border: `1px solid ${C.border}` }}>
                <span style={s.tag(statusColor(trackedTask.status))}>{trackedTask.result?.status?.toUpperCase() || trackedTask.status?.toUpperCase() || "UNKNOWN"}</span>
        ...
        <div style={s.card}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "14px" }}>
            ...
            <div style={{ display: "flex", gap: "8px" }}>
              {deadTasks.length > 0 && <button style={{ ...s.btnDanger, color: C.blue, borderColor: C.border }} onClick={replayDead}>Replay all</button>}
              {deadTasks.length > 0 && <button style={s.btnDanger} onClick={clearDead}>Clear all</button>}
            </div>
          </div>
          {deadTasks.length === 0 ? (
